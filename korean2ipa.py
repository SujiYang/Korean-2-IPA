...
def korOrthoSyllParser(Input):
    ...
    for syll in syll_list:
        ...
        if re.match(".*[ㄱ-ㅎㅏ-ㅣ가-힣]+.*", syll) is not None:
            ...
            #print('초성: {}'.format(initials[i]))
            
            m = int((code - (initial * i)) / mid)
            parsed.append(mids[m])
            #print('중성: {}'.format(mids[m]))
...
def primary():
    ...
    # parse str into initial,mid,final syllable segments (list of characters)
    seg_list = list(korOrthoSyllParser(Input))
    #print(seg_list)
